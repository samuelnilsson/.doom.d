           ...
           `(("llama-swap" .
              (("api" . "openai-chat")
               ...
               ("models" . (("qwen3.5-9b" . ,(make-hash-table))
                            ...
                            ("qwen3-coder-next" . ,(make-hash-table))))))))
         (when work-p
           `(("github-copilot" .
              (("models" .
...
Add project-specific agents directory if in a project.
...
;; Add project-specific agents directory when entering a project
(defun my/gptel-agent-add-project-dirs ()
  "Add project-specific .gptel-agents directory if it exists."
  (when-let ((project (project-current)))
    (let ((project-agents (expand-file-name ".gptel-agents" (project-root project))))
      (when (and project-agents (file-exists-p project-agents)
                 (not (member project-agents gptel-agent-dirs)))
        (add-to-list 'gptel-agent-dirs project-agents)))))

(add-hook 'project-switch-hook #'my/gptel-agent-add-project-dirs)

;; Add global skills directory for git-commit skill (last to prioritize)
(setq gptel-agent-skill-dirs
      (append gptel-agent-skill-dirs '("~/.doom.d/global-skills/")))
#+end_src
