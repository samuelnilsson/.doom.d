...
Configure MCP servers for gptel tool use. =atlassian-rovo= and =github= are only
registered when =DOOM_GPTEL_MAC= is set, as they require network access and credentials
only available on that machine.
