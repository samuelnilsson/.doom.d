...
(use-package! gptel
  ...
  (defun gptel-for-text ()
    ...
    (if (string-equal (getenv "DOOM_GPTEL_MAC") "t")
        (setq-local gptel-model 'qwen3-next:latest))
    (if (string-equal (getenv "DOOM_GPTEL_HOME") "t")
        (setq-local gptel-model 'phi4-mini:latest))
  ...
  (defun gptel-for-programming ()
    (setq-local gptel-backend (gptel-get-backend "Ollama"))
    (if (string-equal (getenv "DOOM_GPTEL_MAC") "t")
        (setq-local gptel-model 'qwen3-coder-next:latest))
    (if (string-equal (getenv "DOOM_GPTEL_HOME") "t")
        (setq-local gptel-model 'qwen2.5-coder:3b))
    (setq-local gptel-system
  ...
  (if (string-equal (getenv "DOOM_GPTEL_MAC") "t")
      (progn
        (gptel-make-ollama "Ollama"
          ...
          :stream t
          :models '(qwen3-coder-next:latest qwen3-next:latest))
  ...
  (if (string-equal (getenv "DOOM_GPTEL_HOME") "t")
      (gptel-make-ollama "Ollama"
        :host "localhost:11434"
        :stream t
        :models '(qwen2.5-coder:3b phi4-mini:latest)))
  (if (string-equal (getenv "DOOM_GPTEL_MAC") "t")
      (setq gptel-model 'qwen3-coder-next:latest
...
Configure MCP servers for gptel tool use.
Filesystem and web fetch are handled by =gptel-agent= (see [[*Tools]]).
           ...
           `(,@(when (string-equal (getenv "DOOM_GPTEL_MAC") "t")
                 '(("atlassian-rovo" . (:command "npx"
                   ...
                   ("github" . (:command "npx"
                                ...
                                :env (:GITHUB_PERSONAL_ACCESS_TOKEN ,(string-trim (shell-command-to-string "gh auth token")))))))))
...
Use =gptel-agent= for a batteries-included set of native Emacs tools: filesystem,
web fetch, bash, and Emacs introspection. No external processes needed.

#+begin_src emacs-lisp :tangle yes
(use-package! gptel-agent
  :after gptel
  :config
  (setq gptel-agent-allowed-dirs
        (mapcar #'expand-file-name '("~/src" "~/Downloads" "~/.doom.d")))
  (gptel-agent-update))
#+end_src
...
(use-package! gptel-magit
  ...
  :config
  (setq gptel-magit-backend (gptel-get-backend "Ollama"))
  (if (string-equal (getenv "DOOM_GPTEL_MAC") "t")
        (setq gptel-magit-model 'qwen3-coder-next:latest))
  (if (string-equal (getenv "DOOM_GPTEL_HOME") "t")
        (setq gptel-magit-model 'qwen2.5-coder:3b)))
#+end_src
...
(use-package! minuet
  ...
  (if (string-equal (getenv "DOOM_GPTEL_MAC") "t")
      (plist-put minuet-openai-fim-compatible-options :model "qwen3-coder-next:latest"))
  (if (string-equal (getenv "DOOM_GPTEL_HOME") "t")
